...
solv.min_trust_box_size = 1e-4
solv.min_approx_improve = 1e-4
solv.max_iter = 50
...
def test_prob(ut, x0, x_true, f=zerofunc, g=neginffunc, h=zerofunc,
    ...
    cnts = []
    nonlin_ineq = LEqExpr(Expr(g), np.zeros(g(0).shape))
    nonlin_ineq = BoundExpr(nonlin_ineq, var)
    cnts.append(nonlin_ineq)
    nonlin_eq = EqExpr(Expr(h), np.zeros(g(0).shape))
...
class TestSolver(unittest.TestCase):
    ...
    def test_prob0(self):
        ...
        g = lambda x: np.array([[3 - x[0] - x[1]]])
        x_true = np.array([[1.5],[1.5]])
        test_prob(self, x0, x_true, f=f, g=g)

    def test_prob1(self):
        x0 = np.array([[-2.0],[1.0]])
        f = lambda x: np.array([[(x[1]-x[0]**2)**2 + (1-x[0])**2]])
        g = lambda x: np.array([[-1.5 - x[1]]])
        x_true = np.array([[1.0],[1.0]])
        test_prob(self, x0, x_true, f=f, g=g)

    def test_prob2(self):
        x0 = np.array([[10.0],[1.0]])
        f = lambda x: np.array([[x[1] + 1e-5 + (x[1]-x[0])**2]])
    ...
    def test_prob3(self):
        x0 = np.array([[10.0],[1.0]])
        f = lambda x: np.array([[(1-x[0])**2]])
        h = lambda x: np.array([[10*(x[1]-x[0]**2)]])
        x_true = np.array([[1.0],[1.0]])
        test_prob(self, x0, x_true, f=f, h=h)

    def test_prob4(self):
        x0 = np.array([[2.0],[2.0]])
        f = lambda x: np.array([[np.log(1+x[0]**2)-x[1]]])
        h = lambda x: np.array([[(1+x[0]**2)**2+x[1]**2-4]])
